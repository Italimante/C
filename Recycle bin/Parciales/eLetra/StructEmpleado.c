...
int emp_comprobarVocalConsonante(letra){
    if(letra==97 || letra==101 || letra==105 || letra == 111 || letra == 117 || letra == 65 || letra == 69 || letra == 73 || letra == 79 || letra == 85){
        return 1;
    }else{
        return 0;
    }
...
sLetra * emp_newLetra (sLetra * this){
    ...
    if(this!=NULL){
        ...
        fflush(stdin);
        scanf("%c",&letra);
        printf("- - [TEST]: LETRA RECIEN INGRESADA: %c\n", letra);
        while(!(letra>=97 & letra<=122) & !(letra>=65 & letra<=90)){
            printf("Solo letras, re-ingrese\n");
            fflush(stdin);
            scanf("%c",&letra);
        }
        this->letra = letra;
...
int generic_menu (void){
    ...
    printf("\n GESTION DE LETRAS\n");
    printline(60);
    printf("\n|1|. Cargar datos.csv y mostrar listado");
    printf("\n|2|. Completar: Vocal y consonante");
    printf("\n|2|. Comprobar: Ingreso de datos");
    //printf("\n|3|. Listar\n");
    printf("\n|0|. Salir\n");
    printline(60);
...
void generic_alta(ArrayList* lista, int* id){
    /*
        Reviso el array la cantidad de vocales y consonantes
        Las guardo en una variable

        Le pido al usuario que ingrese una palabra

        Recorro la palabra para contar las vocales y constantes
        Si en mi lista hay suficientes vocales, la guardo, de lo contrario tiro error
        Y elimino las letras que se usaron

        -------------------

        Pido palabra
        - Recorro la letra de la lista con la primer letra de la palabra
        --
    ...
    */
    ...
    for(i=0; i<len; i++){
        aux=lista->get(lista,i);
        if(emp_getState(aux)!=0){
            vocal = emp_getVocal(aux);
            constante = emp_getConsonante(aux);
...
int main_mostrarLista(ArrayList* lista){
    ...
    if(lista!=NULL){
        len=lista->len(lista);
        printline(60);
        printf("\nLETRA\t| VOCAL\t| CONS\t| NOMBRE\n");
        printline(60);
        for(i=0; i<len; i++){
            aux=lista->get(lista,i);
            if(emp_getState(aux)!=0){
                printf("\n%c \t %d\t %d\t %s\t",emp_getLetra(aux),emp_getVocal(aux),emp_getConsonante(aux),emp_getName(aux));
            }
        }
        printf("\n");
        printline(60);
...
int completarLista(ArrayList * lista){
    char letra;
    int comprobar;
    sLetra* aux;
    int i;
    ...
    for(i=0; i<len; i++){
        aux=lista->get(lista,i);
        if(emp_getState(aux)!=0){
            letra = emp_getLetra(aux);
            comprobar = emp_comprobarVocalConsonante(letra);
            if(comprobar){
                emp_setVocal(aux,1);
                emp_setConsonante(aux,0);
            }else{
                emp_setConsonante(aux,1);
                emp_setVocal(aux,0);
            }
            //lista->add(lista,aux);
        }
