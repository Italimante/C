...
/*
cada vez q egresa se tiene que sumar en un lugar el precio

*/

/*
    No mostrar recaudaciones ahsta que esten egresados

*/
int auto_mostrarEgresados(eAuto listadoA[], int cantA){
    int i;
    int retorno = 0;

    for(i=0;i<cantA;i++){
        if(listadoA[i].estado == EGRESADO){
            auto_mostrarUno(listadoA[i]);
            retorno = 1;
        }else{
            retorno = -1;
        }
...
int auto_egresoAuto(eAuto listaA[], int cantA, eProp listadoP[], int cantP){
    ...
    if(listaA[auxA].estado == OCUPADO){
        ...
        //listaA[auxA].estado = LIBRE; 7750
        listaA[auxA].estado = EGRESADO;

        // o meter un retorno de acutarifa
        retorno = 1;
...
int tarifaMasAlta(eAuto listadoA[], int cantA, eProp listadoP[], int cantP){
    int aux;
    int i;
    int j;
    int auxIdProp;
    ...
    for(i=0;i<cantA;i++){
        for(j=i+1;j<cantA-1;j++)
        if(listadoA[i].estado == EGRESADO){
                if(listadoA[i].horas>listadoA[j].horas){
                    aux = listadoA[j].horas;
                    auxIdProp = listadoA[j].idProp;
                    auxIdAuto = listadoA[j].idAuto;
                }
        }
    }
    ...
    prop_mostrarUno(listadoP[auxIdProp]);
    printf("Horas que estuvo estacionado: %d\n",aux);
}
